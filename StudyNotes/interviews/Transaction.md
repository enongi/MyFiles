...
置顶  文章标签： [Spring](https://so.csdn.net/so/search/s.do?q=Spring&t=blog)[事务管理](https://so.csdn.net/so/search/s.do?q=事务管理&t=blog)[传播行为](https://so.csdn.net/so/search/s.do?q=传播行为&t=blog)[事务配置](https://so.csdn.net/so/search/s.do?q=事务配置&t=blog) 更多

版权声明：本文为博主原创文章，遵循[ CC 4.0 BY-SA ](http://creativecommons.org/licenses/by-sa/4.0/)版权协议，转载请附上原文出处链接和本声明。本文链接：https://blog.csdn.net/Trigl/article/details/50968079
...
**一、事务的作用**
 ...
 持久性 Durability） ：指一个事务一旦被提交，它对数据库中数据的改变就是永久性的，即时数据库发生故障也不应该对其有任何影响。
...
```xml
``"txManager"` `class``=``"..."``>`` ```"sessionFactory"``>`` `````"txManager"``/>`
```
...
![这里写图片描述](https://img-blog.csdn.net/20160324011156424)
...
![这里写图片描述](https://img-blog.csdn.net/20160325003448793)
